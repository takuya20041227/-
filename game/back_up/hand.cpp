...
//外部変数
//-----------------------------------------------------------------------------------------------------------------------
SINT32 unique_box[ 1024 ];


void unique_box_init( void )
{
	SINT32 i;
	for( i = 0; i < 1024; i++ )
		unique_box[ i ] = -999;
...
void unique_id_get(TASK *actp)
{
	SINT32 i;
	for( i = 0; unique_box[ i ] != -999; i++ )					//エンコードを見つけに行く
		if( unique_box[ i ] == actp->unique_id )
			return;
	
	unique_box[ i ] = actp->unique_id;							//ユニークナンバーを入れる
}
...
{            
	...
	switch( ap->SCENE )			
	{
		case 0:								//地面に当たるまで飛ぶ
			...
			if( ap->pos[ Y ] <= GROUND || ap->TIMER > 45 )		//地面についたら
			{
				ap->vec[X] = -ap->vec[X];
				ap->vec[Y] = -ap->vec[Y];
				ap->vec[Z] = -ap->vec[Z];
		...
		case 10:							//自機に戻っていく
			...
			//ap->ang[X] = -ang_x;
			ap->ang[Y] = ang_y;
			ap->vec[X] = SOZ_get_sin( ang_y ) * HAND_SPEED * 1.25f;	//ベクトルの計算
			ap->vec[Y] = SOZ_get_sin( ang_x ) * HAND_SPEED * 1.25f;	//ベクトルの計算
			ap->vec[Z] = SOZ_get_sin( ang_z ) * HAND_SPEED * 1.25f;	//ベクトルの計算
		...
		case 20:
			...
			if( SOZ_Mouse_Button( 1 ) == 1 || game_type == 501 && rp->click[1] != 0 || index == 0 )
			{
				rp->click[1] = 1;
				catch_switch = 0;
				catch_shot( ap );
